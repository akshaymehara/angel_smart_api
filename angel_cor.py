...
exchange = "NSE"
runcount = 0
fund_per_trade = 10000


def GettingLtpData(script, token, order):
    LTP = obj.ltpData(exchange, script, token)
    ltp = LTP["data"]["ltp"]
    quantity = int(fund_per_trade / ltp)
    # if (order == "BUY") and (script not in traded_dict):
    #     traded_dict[script] = quantity
    # if (order == "SELL") and (script not in traded_dict):
    #     traded_dict[script] = -quantity
    traded_dict[script] = json.loads({
        "price": ltp,
        "quantity": quantity,
        "order": order
    })
    ...
    print(
        f"{order} order Place for {script} with quantity : {quantity} at : {datetime.datetime.now()} with Order id {orderId}"
    )
    print(traded_dict)
    ...
    try:
        for script, token in script_list.items():
            ...
            if hist_data != None:
                ...
                # print(df)
                if not df.empty:
                    if (df["high"].values[-1] == df["open"].values[-1]) and (
                            script not in sell_traded_stock):
                        sell_traded_stock.append(script)
                        GettingLtpData(script, token, "SELL")

                    if (df["low"].values[-1] == df["open"].values[-1]) and (
                            script not in buy_traded_stock):
